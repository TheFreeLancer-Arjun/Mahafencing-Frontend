...
const HomePageFeatures = ({ data, title, columns = 2 }) => {
  return (
    <div className="bg-yellow-50 flex flex-col justify-center items-center w-full border-t-[20px] border border-t-[#06B4DB] mb-20">
      <h1 className="text-center mt-5 text-2xl font-bold bg-yellow-50 border-b-[3px] border-r-[3px] border border-[#06B4DB] rounded-xl px-7 py-2 text-black uppercase">
        {title}
      </h1>
          ...
          <Link to={card.pageLink || "#"} key={index} className="w-full">
            <div className="rounded-2xl shadow-lg overflow-hidden p-0 bg-white border-[2px] border-black">
              ...
              <div className="text-center mt-2 pt-4 bg-[#F6F2DF] text-black h-[4cm] border border-[#06B4DB] border-t-[10px]">
                ...
                {card.description && (
                  <p className="text-sm px-4 mt-1">{card.description}</p>
                )}
              </div>
            </div>
          </Link>
